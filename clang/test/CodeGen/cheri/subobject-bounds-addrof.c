...
// REQUIRES: asserts
// RUN: %cheri_purecap_cc1 -cheri-bounds=aggressive -O2 -std=c11 -emit-llvm %s -o %t.ll -Rcheri-subobject-bounds -verify
// RUN: %cheri_FileCheck %s -check-prefixes CHECK -input-file=%t.ll
// RUN: %cheri_purecap_cc1 -cheri-bounds=aggressive -DUSE_BUITLTIN_ADDROF -O2 -std=c11 -emit-llvm %s -o - -mllvm -debug-only=cheri-bounds -mllvm -stats 2>%t.dbg | %cheri_FileCheck %s -check-prefixes CHECK
...
void test_subobject_addrof_basic(struct WithNested* s) {
  ...
  // CHECK: [[BOUNDED_FLOAT:%.+]] = tail call addrspace(200) ptr addrspace(200) @llvm.cheri.cap.bounds.set.i64(ptr addrspace(200) {{.+}}, i64 4)
}
...
void test_fnptr(struct ContainsFnPtr *s) {
  ...
  do_stuff_with_fn_ptr_ptr(&s->fn_ptr); // expected-remark-re {{setting sub-object bounds for field 'fn_ptr' (pointer to 'fn_ptr_ty' (aka 'void (*)(void)')) to {{16|32}} bytes}}
