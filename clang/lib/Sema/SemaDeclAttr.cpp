...
static void handleCHERIMethodClass(Sema &S, Decl *D, const ParsedAttr &Attr) {
  auto II = Attr.getArgAsIdent(0)->getIdentifierInfo();
...
static void handleMinimumStack(Sema &S, Decl *D, const ParsedAttr &Attr) {
  uint32_t size;
  if ((Attr.getNumArgs() != 1) ||
      !S.checkUInt32Argument(Attr, Attr.getArgAsExpr(0), size))
    return;
...
void Sema::ProcessDeclAttributes(Scope *S, Decl *D, const Declarator &PD) {
  ...
  auto ProcessAttributesWithSliding =
      [&](const ParsedAttributesView &Src,
        ...
        ProcessDeclAttributeList(S, D, NonSlidingAttrs, DAL_Unspecified,
                                 Options);
      };
