...
private:
  // bits:     |0 1 2|3|4 .. 5|6  ..  8|9|10|11  ...   31|32 ... 63|
  //           |C R V|U|GCAttr|Lifetime|I| O|AddressSpace| PtrAuth |
  uint64_t Mask = 0;
  ...
  static const uint32_t OMask = 0x400;
  static const uint32_t OShift = 10;
  static constexpr uint64_t AddressSpaceMask =
      ~(CVRMask | UMask | GCAttrMask | LifetimeMask | PtrAuthMask | IMask | OMask);
  static constexpr uint64_t AddressSpaceShift = 11;
};
...
class ConstantArrayType : public ArrayType {
  ...
  ConstantArrayType(QualType Et, QualType Can, uint64_t Width, uint64_t Sz,
      ...
      : ArrayType(ConstantArray, Et, Can, SM, TQ, PIK, nullptr), Size(Sz) {
...
protected:
  ConstantArrayType(TypeClass Tc, const ConstantArrayType *ATy, QualType Can,
      ...
      : ArrayType(Tc, ATy->getElementType(), Can, ATy->getSizeModifier(),
                  ATy->getIndexTypeQualifiers().getAsOpaqueValue(), PIK, nullptr) {
...
class ArrayParameterType : public ConstantArrayType {
  ...
  ArrayParameterType(const ConstantArrayType *ATy, QualType CanTy)
      : ConstantArrayType(ArrayParameter, ATy, CanTy, std::nullopt) {}
