...
ArrayRef<MCPhysReg> RISCV::getArgGPRs(const RISCVABI::ABI ABI) {
  ...
  if (ABI == RISCVABI::ABI_ILP32E || ABI == RISCVABI::ABI_LP64E ||
      ABI == RISCVABI::ABI_IL32PC64E || ABI == RISCVABI::ABI_CHERIOT ||
...
static bool CC_RISCVAssign2XLen(unsigned XLen, CCState &State, bool IsPureCapVarArgs,
                                CCValAssign VA1,
...
bool llvm::CC_RISCV(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  MVT CLenVT = Subtarget.hasVendorXCheri() ? Subtarget.typeForCapabilities()
                                    : MVT();
  ...
  if (LocVT == MVT::f64 && !UseGPRForF64 && !IsPureCapVarArgs) {
  ...
  if (XLen == 32 && LocVT == MVT::f64) {
    ...
    Register Reg = IsPureCapVarArgs ? MCRegister{0} : State.AllocateReg(ArgGPRs);
    if (!Reg) {
  ...
  if (ValVT.isScalarInteger() && ArgFlags.isSplitEnd() &&
    ...
    return CC_RISCVAssign2XLen(
        ...
        ABI == RISCVABI::ABI_ILP32E || ABI == RISCVABI::ABI_LP64E || ABI == RISCVABI::ABI_CHERIOT || ABI == RISCVABI::ABI_CHERIOT_BAREMETAL);
  }
  ...
  } else if (RISCVABI::isCheriPureCapABI(ABI) && !PendingLocs.empty()) {
    // Allocate a cap register to hold the pointer to indirectly-passed
    // split arguments.
