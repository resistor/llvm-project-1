...
static Defined *addAbsolute(Ctx& ctx, StringRef name) {
  Symbol *sym = ctx.symtab->addSymbol(Defined{
      ctx, nullptr, name, STB_GLOBAL, STV_HIDDEN, STT_NOTYPE, 0, 0, nullptr});
...
template <class ELFT> void Writer<ELFT>::addSectionSymbols() {
  for (SectionCommand *cmd : ctx.script->sectionCommands) {
    ...
    auto *sym = makeDefined(ctx, isec->file, "", STB_LOCAL,
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  ...
  for (Symbol *s : ctx.symtab->getSymbols()) {
    ...
    if (const OutputSection *outSec = reg->getOutputSection())
      // XXXAR: ctx.out.elfHeader is a special output section and will never be
      // marked as live. We still need keep symbols pointing there since they
      // will then point to the first output section
      // See assignment above:   ctx.out.elfHeader->SectionIndex = 1;
...
template <class ELFT> void Writer<ELFT>::addStartEndSymbols() {
  ...
  auto define = [=](StringRef start, StringRef end, OutputSection *os) {
    if (os) {
      ...
      addOptionalRegular(ctx, start, ctx.out.elfHeader.get(), 0, STV_HIDDEN,
                         /*canBeSectionStart=*/false);
      addOptionalRegular(ctx, end, ctx.out.elfHeader.get(), 0, STV_HIDDEN,
                         /*canBeSectionStart=*/false);
    }
  };
...
Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  if (cheriBounds && cheriBounds->firstSec) {
    ctx.in.cheriBounds = cheriBounds.get();
...
template <class ELFT> void Writer<ELFT>::openFile() {
  ...
  ctx.bufferSize = buffer->getBufferSize();
}
...
class CompartmentReportWriter {
   ...
   */
  void addImports(Ctx &ctx, StringRef compartmentName,
                  ArrayRef<ImportTableEntry> table) {
   ...
   */
  void processExports(Ctx &ctx, OutputSection *sec, bool isLibrary) {
   ...
   */
  void addOutputSection(Ctx &ctx, OutputSection *sec) {
    ...
    if (StringSwitch<bool>(sec->name)
      ...
      for (auto *inSec : getInputSections(*sec, storage)) {
        ...
        if (inSec->name == ".compartment_import_table") {
          addImports(ctx, compartmentName,
...
public:
   ...
   */
  CompartmentReportWriter(Ctx &ctx, StringRef fileName, uint8_t *buffer,
                          size_t bufferSize)
      : Json({{"file", fileName},
              {"core", json::Object()},
    ...
    if (ctx.arg.shouldEmitCompartmentReport()) {
      for (OutputSection *sec : ctx.outputSections)
        if (sec->name == ".compartment_export_tables") {
          processExports(ctx, sec, false);
        } else if (sec->name == ".library_export_tables") {
          processExports(ctx, sec, true);
        } else if (sec->name == ".sealed_objects")
          sealedObjects = sec;

      if (Symbol *sym = ctx.symtab->find("__shared_objects_start"))
        preSharedObjectStart = sym->getVA(ctx);
      if (Symbol *sym = ctx.symtab->find("__shared_objects_end"))
        preSharedObjectEnd = sym->getVA(ctx);
      for (OutputSection *sec : ctx.outputSections)
        addOutputSection(ctx, sec);
      ...
      writeToFile(ctx.arg.compartmentReportFile);
    }
  }
};
...
template <class ELFT> void Writer<ELFT>::writeSections() {
  ...
  CompartmentReportWriter{ctx, ctx.arg.outputFile, ctx.bufferStart,
                          ctx.bufferSize};
