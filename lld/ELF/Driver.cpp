...
static void readConfigs(Ctx &ctx, opt::InputArgList &args) {
  ...
  ctx.arg.compartment = args.hasArg(OPT_compartment);
  ctx.arg.compartmentReportFile = args.getLastArgValue(OPT_compartment_report);
  assert(ctx.arg.searchPaths.empty() && "Should not be set yet!");
  ctx.arg.resolveGroups = !args.hasArg(OPT_relocatable) ||
                          args.hasArg(OPT_force_group_allocation) ||
                          ctx.arg.compartment;
